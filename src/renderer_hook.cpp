...
struct smoothpan_renderer_2d_hook : public df::renderer_2d {
    ...
    DEFINE_VMETHOD_INTERPOSE(void, update_full_viewport, (df::graphic_viewportst* vp)) {
        ...
        // 3.24.0 retained-frame compositor: capture the tile pass into our own
        // render target.  Must come BEFORE the map clip so the clip applies to
        // the capture texture.  No-op when the compositor is off/disabled.
        bool captured = false;
        if (shift_pass && is_enabled && !trace_is_active() &&
            sdl_shift_mode_active() && compositor_active()) {
            compositor_pass_begin(sdl_renderer, vp == main_vp);
            captured = compositor_capturing();
        }
        ...
        // Composite the captured layer back onto DF's target.  Done after the
        // gate is closed, and the composite blit is issued through the
        // trampoline, so it is never re-shifted or re-captured.
        if (captured) compositor_pass_end(sdl_renderer);
    ...
    DEFINE_VMETHOD_INTERPOSE(void, render, ()) {
        ...
        // Fallback: if nothing the HUD drew ended the captured map layer,
        // composite it now (before present) and hand DF its target back.
        if (compositor_capturing()) {
            void* sdl_renderer = nullptr;
            auto* r2d = virtual_cast<df::renderer_2d>(
                df::global::enabler ? df::global::enabler->renderer : nullptr);
            if (r2d) sdl_renderer = r2d->sdl_renderer;
            compositor_render_end(sdl_renderer);
        }
        g_in_main_viewport_update.store(false, std::memory_order_relaxed);
